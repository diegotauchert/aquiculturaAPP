...
@if($categoria->id)
@section('title', __('gestor.edicao') . ' - ' . __('gestor_categoria_post.agenda'))
@else
@section('title', __('gestor.novo') . ' - ' . __('gestor_categoria_post.agenda'))
@endif
...
<form method="POST" action="{{ ($categoria->id ? route('gestor.agendas-categorias.update', $categoria->id) : route('gestor.agendas-categorias.store')) }}">
    @if($categoria->id)
    @method('PUT')
    @endif
