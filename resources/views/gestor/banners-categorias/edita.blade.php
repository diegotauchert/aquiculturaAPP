...
@if($categoria->id)
@section('title', __('gestor.edicao') . ' - ' . __('gestor_categoria_post.banner'))
@else
@section('title', __('gestor.novo') . ' - ' . __('gestor_categoria_post.banner'))
@endif
...
<form method="POST" action="{{ ($categoria->id ? route('gestor.banners-categorias.update', $categoria->id) : route('gestor.banners-categorias.store')) }}">
    @if($categoria->id)
    @method('PUT')
    @endif
