...
@if($categoria->id)
@section('title', __('gestor.edicao') . ' - ' . __('gestor_categoria_post.titulo'))
@else
@section('title', __('gestor.novo') . ' - ' . __('gestor_categoria_post.titulo'))
@endif
...
<form method="POST" action="{{ ($categoria->id ? route('gestor.categorias-posts.update', $categoria->id) : route('gestor.categorias-posts.store')) }}">
    @if($categoria->id)
    @method('PUT')
    @endif
