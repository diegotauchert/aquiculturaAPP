...
@if($categoria->id)
@section('title', __('gestor.edicao') . ' - ' . __('gestor_categoria_post.cobertura'))
@else
@section('title', __('gestor.novo') . ' - ' . __('gestor_categoria_post.cobertura'))
@endif
...
<form method="POST" action="{{ ($categoria->id ? route('gestor.coberturas-categorias.update', $categoria->id) : route('gestor.coberturas-categorias.store')) }}">
    @if($categoria->id)
    @method('PUT')
    @endif
