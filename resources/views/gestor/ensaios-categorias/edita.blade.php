...
@if($categoria->id)
@section('title', __('gestor.edicao') . ' - ' . __('gestor_categoria_post.ensaio'))
@else
@section('title', __('gestor.novo') . ' - ' . __('gestor_categoria_post.ensaio'))
@endif
...
<form method="POST" action="{{ ($categoria->id ? route('gestor.ensaios-categorias.update', $categoria->id) : route('gestor.ensaios-categorias.store')) }}">
    @if($categoria->id)
    @method('PUT')
    @endif
