...
@if($categoria->id)
@section('title', __('gestor.edicao') . ' - ' . __('gestor_categoria_post.vaipraonde'))
@else
@section('title', __('gestor.novo') . ' - ' . __('gestor_categoria_post.vaipraonde'))
@endif
...
<form method="POST" action="{{ ($categoria->id ? route('gestor.vaipraonde-categorias.update', $categoria->id) : route('gestor.vaipraonde-categorias.store')) }}">
    @if($categoria->id)
    @method('PUT')
    @endif
