...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ ModelConfig::find('titulo')->valor ?? config('app.name', '') }}</title>
            ...
            <table border="0" cellpadding="0" cellspacing="0">
                <tr>
                    <td class="text-center">
                        <p>&nbsp;</p>
                        <p>
                            <a title="{{ (app('config')->get('app')['dev'] ? app('config')->get('app')['dev'] : '') }}" href="{{ url('/') }}">
                            <img src="{{ asset(mix('images/logo.png')) }}" alt="{{ (app('config')->get('app')['dev'] ? app('config')->get('app')['dev'] : '') }}" width="200" border="0">
                            </a>
                        </p>
                    </td>
                </tr>
